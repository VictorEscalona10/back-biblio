...
def update_book_page(page: ft.Page, body_column: ft.Column, user_email: str):
    ...
    def submit_book(e):
        ...
        if response.status_code == 200:
            message_label.value = "Libro actualizado correctamente"
            message_label.color = ft.colors.GREEN
            page.snack_bar = ft.SnackBar(ft.Text("Libro actualizado correctamente"))
            page.snack_bar.open = True
        else:
            message_label.value = "Error al actualizar el libro"
    ...
    body_column.controls.extend([
        ft.Text("Actualizar Libro", size=24, weight=ft.FontWeight.BOLD),
        title_field,
        author_field,
        year_field,
        link_field,
